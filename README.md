...
Microsoft SSO 로 로그인하는 사내 웹앱. 임직원이 **직접 손으로 그린 물고기**를 **공용 어항**에 풀어놓고, 다른 사람이 그린 물고기가 함께 헤엄치는 모습을 **실시간**으로 감상·상호작용하는 사내 협업/휴식 경험을 제공합니다.

(초기에는 Teams 탭 앱으로 배포하려 했으나, 일반 브라우저 + MSAL 리다이렉트 로그인으로 전환했습니다. 로그인은 회사 Microsoft(Entra) 계정만 허용됩니다.)
...
```
fish-tank/
  client/   React + Vite 프론트엔드 (Microsoft SSO 로그인, 드로잉/어항 렌더링)
  server/   Node.js + Express 백엔드 (SSO 토큰 검증, 물고기 CRUD, 실시간)
  .env.example  필요한 환경 변수 문서 (실제 값은 .env 에)
```

## 기술 스택

- **프론트엔드**: React 18, Vite, `@azure/msal-browser` (Microsoft SSO 리다이렉트 로그인)
- **백엔드**: Node.js, Express, `jose` (SSO JWT/JWKS 검증), `pg` (PostgreSQL)
...
## 로컬 개발용 로그인 우회 (실제 로그인 없이 시험)

Azure 앱 등록 값 없이 로컬에서 로그인·물고기 그리기를 시험하려면 개발 전용 우회를 켤 수 있습니다.

`server/.env` 와 `client/.env`(또는 각 워크스페이스에서 읽는 `.env`)에 다음을 설정합니다.

```
DEV_AUTH_BYPASS=1          # 서버: 모든 요청을 고정 개발 신원으로 인증
VITE_DEV_AUTH_BYPASS=1     # 클라: MSAL 로그인 대신 개발 신원으로 바로 로그인
```
...
[경고] 이 우회는 **개발 전용**입니다.
- 두 플래그 모두 기본값은 비활성이며, 설정하지 않으면 실제 Microsoft SSO 검증을 사용합니다.
...
- [x] **M1 — 인증 및 앱 셸**: Microsoft SSO 세션 획득(클라이언트), 서버 측 토큰 검증 경계,
...
모든 쓰기 작업(물고기 추가·삭제·먹이주기)은 **서버 측에서 Microsoft SSO 토큰을 검증**한 인증 신원만
